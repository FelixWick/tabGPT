...
if __name__ == '__main__':
    ...
    train_targets = rm.df_train[rm.target_column]
    val_targets = rm.df_val[rm.target_column]
    ...
    if in_memory:
        ...
        train_dataset =  load_datasets(dataset_loader=[rm], mode='train')[0]
    ...
    # inference
    df_train = predict(model, DataLoader(train_dataset, batch_size=32), rm.df_train)
    evaluation(df_train[rm.target_column], df_train["yhat"])
    ...
    if in_memory:
        ...
        val_dataset =  load_datasets(dataset_loader=[rm], mode='val')[0]


    df_val = predict(model, DataLoader(val_dataset, batch_size=32), rm.df_val)
    evaluation(df_val[rm.target_column], df_val["yhat"])
