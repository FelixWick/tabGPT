...
if __name__ == '__main__':
    ...
    in_memory = False
    files_generated = True # only used in file-approach
    ...
    train_targets = fd.df_train[fd.target_column]
    val_targets = fd.df_val[fd.target_column]
    ...
    if in_memory:
        ...
        train_dataset =  load_datasets(dataset_loader=[fd], mode='train')[0]
    ...
    # inference
    df_train = predict(model, DataLoader(train_dataset, batch_size=32), fd.df_train)
    evaluation(df_train[fd.target_column], df_train["yhat"])
    ...
    if in_memory:
        ...
        val_dataset =  load_datasets(dataset_loader=[fd], mode='val')[0]


    df_val = predict(model, DataLoader(val_dataset, batch_size=32), fd.df_val)
    evaluation(df_val[fd.target_column], df_val["yhat"])
