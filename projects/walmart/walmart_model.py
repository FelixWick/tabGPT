...
if __name__ == '__main__':
    ...
    train_targets = wm.df_train[wm.target_column]
    val_targets = wm.df_val[wm.target_column]
    ...
    if in_memory:
        ...
        train_dataset =  load_datasets(dataset_loader=[wm], mode='train')[0]
    ...
    # inference
    df_train = predict(model, DataLoader(train_dataset, batch_size=32), wm.df_train)
    evaluation(df_train[wm.target_column], df_train["yhat"])
    ...
    if in_memory:
        ...
        val_dataset =  load_datasets(dataset_loader=[wm], mode='val')[0]


    df_val = predict(model, DataLoader(val_dataset, batch_size=32), wm.df_val)
    evaluation(df_val[wm.target_column], df_val["yhat"])
